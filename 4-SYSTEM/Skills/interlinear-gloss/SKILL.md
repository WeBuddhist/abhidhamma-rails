...
A gloss block is the **token-level alignment** that all downstream bilingual glossary work depends on. `glossary-extract-raw` reads these files to find keyword renderings; `glb` ... (Wait, it says `\glc` in the text?)
Actually, looking at the original: "`glossary-extract-raw` reads these files to find keyword renderings; `local-wiki-article` cites them when documenting how a term is rendered across translations; `glossary-select` consults them when judging whether an attested rendering meets a track's requirements."
Okay, I'll just replace the tags.
...
- [ ] No `glb` cell contains a Pali word or a term absent from the corresponding `\ex` line
