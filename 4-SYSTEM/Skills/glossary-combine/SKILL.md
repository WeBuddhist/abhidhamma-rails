...
description: Merge every raw per-source glossary in 2-RAILS/Glossaries/Raw/ that shares one language pair into a single consolidated glossary at 2-RAILS/Glossaries/<source-lang>-<target-lang>.md. Each keyword's row shows every attested rendering, side by side, with the source that uses it and a frequency count. This is what glossary-select reads.
---

# glossary-combine

This skill takes the raw glossaries that `glossary-extract-raw` produced — one per translation source — and merges them into a single **consolidated glossary** per language pair. The consolidated file is descriptive: it tells you, for each source-language keyword, every distinct rendering attested across the corpus and which sources use which rendering.

The consolidated glossary is the menu that `glossary-select` chooses from when building a per-track glossary.
...
- **Raw glossaries** — all files under `2-RAILS/Glossaries/Raw/` whose `language_pair` frontmatter field matches the requested pair.
...
```
2-RAILS/Glossaries/<source-lang>-<target-lang>.md
```

(e.g. `2-RAILS/Glossaries/pi-en.md`). If the file exists, regenerate it from the current raw inputs — manual edits to the consolidated file are discouraged; consolidate from raw instead.
...
# Consolidated glossary — <source-language> → <target-language>
...
2. **Regenerate, don't edit.** The consolidated file is a derived artefact. If the data is wrong, fix the raw glossary, then re-run this skill. Manual edits get overwritten.
...
`scripts/combine_glossaries.py` reads the raw glossaries with the requested `language_pair`, walks each `##` heading (one per keyword) and its rendering table, merges them, and writes the consolidated output. It does not invent renderings, it does not invent sense splits, and it does not invent Local-Wiki links — those come from existing files.
