...
1. 🧭 **Today's Chanting Guide** — a short note orienting us to the day's passage and how it connects to the path. **Length cap: ~180 words** (two short paragraphs + one closing question). **Reading level: Flesch-Kincaid grade ≤ 9** (8th-grade — matches the audience profile, which is calibrated for international readers with varied English literacy). Average sentence length 12–15 words; common words over Latinate ones; Pāli terms are welcome but each is glossed plainly the first time it appears in the section; no figurative English idioms; no internal-architecture references like "the rail says" — translate rail content into reader-facing language.
...
5. 🔑 **Pāli Word of the Day** — one key word to carry as an anchor. **Length cap: ~140 words** (pronunciation + "In today's passage" + "Why it matters"). **Reading level: same target as §1 — Flesch-Kincaid grade ≤ 9, average sentence length 12–15 words.** Casual practitioner, not Buddhist Studies seminar. The Pāli word being foregrounded is itself a technical term; that's the point of the section. Other technical terms that come up are welcome too, as long as each is glossed plainly the first time. No internal-architecture references.
...
The Daily Tipitaka is published in multiple languages so practitioners can read the meaning in their own tongue while still chanting in Pāli. Each language is a top-level subfolder of the plan, with day files under `<lang>/days/`:

- `pi/days/` — Pāli (source language; meaning gloss kept minimal, Pāli analysis foregrounded)
- `en/days/` — English
- `bo/days/` — Tibetan
- `zh/days/` — Chinese
- `hi/days/` — Hindi

Currently scaffolded: **day-001 through day-010** in each language. The remaining 190 days will be added plan by plan from `en/days/_template/day-template.md` as content is written.
...
**One track per language, across all three books.** Every English day draws §4 from `en-Contemporary-English-Abhidhamma/`, whichever book it falls in — `en-dhammasangani-ai.md` for Book I, `en-vibhanga-ai.md` for Book II, and so on. A day must never draw §4 from a different track than the days around it: the reader is on one continuous journey, and a mid-journey change of register or terminology would read as a break in voice. If a translation arrives labelled for a different audience, check it against this track's `requirements.md` before assuming it needs a track of its own.
...
- `en/schedule.md` — the master calendar mapping day-001 … day-200 to real-world dates, and plans to weeks.
