...
## Notes for the human contributor

After reviewing this draft, move it to:

```
4-SYSTEM/Skills/vault-audit/SKILL.md
```

Then add it to `4-SYSTEM/Skills/SKILLS-CATALOG.md` under a new **Maintenance skills** section, and create `.claude/commands/vault-audit.md` with the standard one-line format. The audit skill will then flag its own absence on the first run if any of these three steps are missed — which is a useful self-check.

To schedule it weekly, use the Cowork scheduler with a prompt like:
> Run the vault-audit skill (read 4-SYSTEM/Skills/vault-audit/SKILL.md and execute it).

Set the schedule to weekly on a day before active collaboration begins (e.g. Saturday or Sunday morning).
