...
This folder holds the **authentic primary materials** the vault is built on: the Pāli Canon root texts, their classical commentaries, existing translations, audio recordings, and reference works. Nothing in this folder is generated by the project. Files are ingested *as received* from their canonical edition and lightly normalised — heading structure, block IDs, frontmatter, internal links — so that downstream rails can cite them precisely.
...
| Bible-style (e.g. Tipiṭaka) | `^book-verse` | `^1-1` |
...
![[1-SOURCES/Text/[lang]-root-text.md#^1-4]]
![[1-SOURCES/Text/[lang]-root-text.md#^1-5]]
...
- **Obsidian:** open the root text frontmatter, follow `related_commentaries` links, navigate to `^6-33` in each commentary file.
- **LLM / resolver script:** reads root text frontmatter `related_*` lists, fetches each file, navigates to the `^6-33` block via transclusion.
...
```markdown
[Ed: uncertain — two witnesses read *dharmadhātu* here. See <edition> p. 47 apparatus.]
```
...
## 14. Checklist for adding a new file
...
- [ ] Verse numbers restart per chapter (except Bible-style books — see annex).
